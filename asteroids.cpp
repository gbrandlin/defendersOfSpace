			...
			if(!WavePowerUp){
				...
				if (ts > 0.1) {
					...
					if (g.nbullets < MAX_BULLETS) {
						...
						//~ b3->pos[0] += xdir*20.0f;
						//~ b3->pos[1] += ydir*20.0f;
						//~ b3->vel[0] += xdir*6.0f + rnd()*0.1;
						//~ b3->vel[1] += ydir*6.0f + rnd()*0.1;
						//~ b3->color[0] = 1.0f;
						//~ b3->color[1] = 1.0f;
						//~ b3->color[2] = 1.0f;
						g.nbullets+=3;
...
void showBullet()
{
	...
	//~ glVertex2f(b->pos[0]-bull, b->pos[1]);
	//~ glVertex2f(b->pos[0]+bull, b->pos[1]);
	//~ glVertex2f(b->pos[0],      b->pos[1]-bull);
	//~ glVertex2f(b->pos[0],      b->pos[1]+bull);
	//~ glColor3f(0.8, 0.8, 0.8);
	//~ glVertex2f(b->pos[0]-bull, b->pos[1]-bull);
	//~ glVertex2f(b->pos[0]-bull, b->pos[1]+bull);
	//~ glVertex2f(b->pos[0]+bull, b->pos[1]-bull);
	//~ glVertex2f(b->pos[0]+bull, b->pos[1]+bull);
	//~ glEnd();
